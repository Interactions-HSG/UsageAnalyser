    ...
    try:
        ...
        for i in furniture_coordinates:
            ...
            for j in range(len(x1)):
                ...
                #evaluate the distance's impact for the furniture usage
                NUMBER_PPL, used_furn_layout, filtered_array = calculate_usage(distance,w,h,x1,y1, i, usage_type)
            ...
            try:
                if Usage_Type == "Warm":
                    ...
                    writer.writerow(
                        {"Timestamp": time.strftime('%b-%d-%Y_%H%M%S', time.localtime()), "Furniture_Type": i, "Usage_Count": COUNT_TABLE[i],"Total_Checks":count,"Usage_Percentage": round((COUNT_TABLE[i] / count)*100,2), "Usage_Type": config.USAGE.get(usage_type), "Room_Occupancy": config.OCCUPANCY.get(occupancy), "Usage amount": NUMBER_,  "Device_ID": get_serial()})
...
def calculate_usage(distance,w,h,x1,y1, i, usage_type):
    """Calculate how many people/objects use the object"""
    NUMBER_PPL = ""
    ct = CentroidTracker()
    trackers = ""
    used_furn_layout = []
    ...
    if (int(distance) < (config.DEFAULT_DISTANCE+(w/2)) or int(distance) < (config.DEFAULT_DISTANCE+(h/2))):
            ...
            #print("trackers coordinates are", trackers)
            print("point2")

            objects = ct.update(trackers)
            print("point3")

            #print("gsamti objects liste", objects)
            NUMBER_PPL = len(objects)
            #print("WIEVIEL Luet", NUMBER_PPL)
            #peopleCount = Neue centroid, aktualisier d ID's. Am schluss wett ich wüsse wv centroids pro möbelstück (i)
            # das heisst am schluss isch d len(objects) d azahl mensche wos benutzed.
            #NUMBER = PEOPLE_COUNTER[i] + NUMBER_PPL
    ...
    return NUMBER_PPL, used_furn_layout, filtered_array
