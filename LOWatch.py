...
def headDetection(fimage):
    ...
    model.setInput(cv2.dnn.blobFromImage(fimage, size=config.inputImageSize, swapRB=True))
...
def startWatching(start_time):
    ...
    out = cv2.VideoWriter('output'+starttimestamp+'.avi', fourcc, 34, (int(ww),int(hw)))
    check,firstFrame=video.read()
    firstFrame=cv2.GaussianBlur(firstFrame,config.blurrSize,0)
    while (int(time.time() - start_time) < capture_duration ):
        check,frame=video.read()
        #cv2.imwrite('train_image_fisheye/'+str(i)+'.jpg',frame)
        #i+=1
        ...
        if check == True: 
            ...
            for (x, y, w, h) in faces:
                roi_blur = frame[y:y+h, x:x+w]
                blur = cv2.GaussianBlur(roi_blur, (101,101), cv2.BORDER_TRANSPARENT )       
                frame[y:y+h, x:x+w] = blur
            (framedetect,changeCoordinates,personCoordinates,RoomStatus, isPerson)=changeDetection(frame,firstFrame)
            cv2.putText(framedetect,'WorkSpace is:',(50,50),cv2.FONT_HERSHEY_SIMPLEX,(1),(0, 0, 255))
            if RoomStatus == "Free":
                cv2.putText(framedetect,RoomStatus,(275,50),cv2.FONT_HERSHEY_SIMPLEX,(1),(0, 255, 0))
            else:
                cv2.putText(framedetect,RoomStatus,(275,50),cv2.FONT_HERSHEY_SIMPLEX,(1),(0, 0, 255))
          
            cv2.putText(framedetect, timestamp, (50,75), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0))
            cv2.putText(framedetect, str(isPerson), (50,100), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0))
    ...
    cv2.imwrite('newCapture'+starttimestamp+'.jpg',framedetect)
    out.release()
    video.release()
