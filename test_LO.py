...
class TestLO(unittest.TestCase):
    ...
    def test_recording(self):
       self.assertEqual(Record.start_recording(self.start_time), self.change)

    '''distanceCal.py'''

    def test_distanceCalculator(self):
        self.assertEqual(distanceCalc.distance_calculator(self.x1, self.y1, 1, self.writer), self.filtered_array)

    def test_iterateNPlot(self):
        self.assertEqual(distanceCalc.iterate(self.coordinates), (self.x1, self.y1))
